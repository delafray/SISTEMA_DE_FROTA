...
export default function PedidosPage() {
  ...
  const handleMarcarPago = async (id: string) => {
    try {
      ...
      if (error) {
        ...
        setTodos(prev => prev.map(p => p.id === id ? { ...p, pago: true, data_pagamento: today } : p));
      }
    } catch (err) {
  ...
  const haystack = useMemo(() => {
    ...
    for (const p of todos) {
      const veiculo   = Array.isArray(p.veiculos) ? p.veiculos[0] : p.veiculos;
      const motorista = Array.isArray(p.motoristas) ? p.motoristas[0] : p.motoristas;
      m.set(p.id, normalizar([
  ...
  const filtrados = useMemo(() => {
    ...
    return todos.filter(p => {
      ...
      if (!mostrarPagos) {
        if ((p.status === "concluido" || p.status === "concluida") && p.pago === true) return false;
      } else {
        if (!((p.status === "concluido" || p.status === "concluida") && p.pago === true)) return false;
        const dateToCompare = p.data_pagamento || p.data_inicio_prevista;
      ...
      return palavras.every(palavra => h.includes(palavra));
    });
  }, [todos, haystack, buscaDeferred, filtroStatus, mostrarPagos, filtroPeriodo, dataInicio, dataFim]);

  const totais = useMemo(() => {
    const concluidos = todos.filter(p => p.status === "concluido" || p.status === "concluida");
    return {
      agendado:          todos.filter(p => p.status === "agendado" || p.status === "agendada").length,
      em_andamento:      todos.filter(p => p.status === "em_andamento").length,
      concluido:         concluidos.length,
      concluidoPago:     concluidos.filter(p => p.pago === true).length,
      concluidoPendente: concluidos.filter(p => p.pago !== true).length,
      receita:           concluidos.reduce((s, p) => s + (p.valor_pedido ?? 0), 0),
      receitaPaga:       concluidos.filter(p => p.pago === true).reduce((s, p) => s + (p.valor_pedido ?? 0), 0),
      receitaPendente:   concluidos.filter(p => p.pago !== true).reduce((s, p) => s + (p.valor_pedido ?? 0), 0),
    };
  }, [todos]);

  const receitaExibida = useMemo(() => {
    return filtrados.reduce((s, p) => s + (p.valor_pedido ?? 0), 0);
  }, [filtrados]);
  ...
  const toolbar = (
    <div style={{ display: "flex", gap: "8px", alignItems: "center", flex: 1, flexWrap: "wrap" }}>
      ...
      <Btn variant={mostrarPagos ? "primary" : "outline"} onClick={() => { setMostrarPagos(!mostrarPagos); if (!mostrarPagos) setFiltroStatus(""); }}>
      ...
      <span style={{ fontSize: "11px", color: "#94a3b8", marginLeft: "auto", whiteSpace: "nowrap" }}>
        {filtrados.length} de {todos.length} pedidos
      </span>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <PageHeader title="Pedidos" count={loading ? undefined : todos.length}>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: "12px" }}>
        <div className="m-kpi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px" }}>
          ...
          <KpiCard label={mostrarPagos ? "Receita Paga (Período)" : "Receita Pendente"}
        ...
        <DataTable count={filtrados.length} label="pedidos" toolbar={toolbar}>
          <thead>
            <tr>
        ...
        <MobileList count={filtrados.length} label="pedidos">
