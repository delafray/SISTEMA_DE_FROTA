...
import { PageHeader, FormSection, FormField, inputStyle, selectStyle, Btn, Alert, DataTable, Th, Td, Tr } from "@/components/ui/ds";
            ...
            <FormSection title="Histórico">
              <div style={{ display: "flex", borderBottom: "1px solid #e2e8f0", marginBottom: "12px" }}>
                {([["abast","Abastecimentos"],["fretes","Fretes"]] as const).map(([key, label]) => (
                  ...
                  }}>{label} ({histTab === key ? (key === "abast" ? abastecimentos.length : freteHist.length) : ""})</button>
                ))}
              </div>
                ...
                freteHist.length === 0
                  ...
                  : <DataTable count={freteHist.length} label="fretes">
                      ...
                      <tbody>
                        {freteHist.map(f => (
                          <Tr key={f.id}>
                            <Td style={{ fontWeight: 500 }}>{f.origem} → {f.destino}</Td>
                            <Td><span style={{ fontSize: "11px", fontWeight: 600 }}>{f.status}</span></Td>
                            <Td>{f.data_coleta_prevista ? new Date(f.data_coleta_prevista + "T00:00:00").toLocaleDateString("pt-BR") : "—"}</Td>
                            <Td style={{ textAlign: "right" }}>{f.km_inicial?.toLocaleString("pt-BR") ?? "—"}</Td>
                            <Td style={{ textAlign: "right" }}>{f.km_final?.toLocaleString("pt-BR") ?? "—"}</Td>
                          </Tr>
                        ))}
                      </tbody>
                    </DataTable>
