...
6. **Escritor genérico do motor de regras** (prioridade do dono, 11/06): hoje a tela só sustenta regras de LEITURA — escrita nova cai em "em breve, não alterei nada" (só KM e status do veículo têm caminho dedicado). Construir: (a) regra declara na tela **campos esperados** (tabela, ação, campos a extrair — ex: Folga = motorista+data); (b) bot extrai, mostra preview padrão, e no "sim" o executor monta INSERT/UPDATE **só com colunas da allowlist** (mesma trava do KM); (c) consulta genérica ganha **soma/período** ("quanto gastei esse mês" hoje lista, não soma); (d) **validador no salvar da regra**: confere se tabela/colunas existem e se o acesso não está bloqueado (RLS/GRANT) — avisa NA HORA em vez de falhar mudo na execução. Objetivo: regra nova 100% pela tela, sem IA de desenvolvimento (proteção contra custo de IA inviável).
7. **Refatoração dos blocos gigantes** (não é feature, é sustentabilidade): `mobile/rota` 1.178 linhas, `ajuste-rota` 911, `pedidos/importar` 891, `InputEnderecoNF` 791, `despacho` 790, `despacho/[id]` ~750. Um por vez, suíte verde entre cada. `messageRouter` (940) fica quieto — já delega bem. `pedidos/novo` × `novo-avancado`: **manter os dois por ora** (decisão do dono 10/06, reavaliar após uso real).
